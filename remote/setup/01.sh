...
# Prompt to enter a password for the PostgreSQL lighten user (rather than hard-coding 
# a password in this script).
read -p "Enter password for lighten DB user: " DB_PASSWORD
...
# Add the new user (and give them sudo priviledges).
...
## Use this instead: curl -L https://github.com/golang-migrate/migrate/releases/download/v4.14.1/migrate.linux-amd64.tar.gz | tar xvz
...
# Set up the lighten DB and create a user account with the password entered earlier.
sudo -i -u postgres psql -c "CREATE DATABASE lighten"
sudo -i -u postgres psql -d lighten -c "CREATE EXTENSION IF NOT EXISTS citext"
sudo -i -u postgres psql -d lighten -c "CREATE ROLE lighten WITH LOGIN PASSWORD '${DB_PASSWORD}'"

# Add a DSN for connecting to the lighten database to the system-wide environment
# variables in the /etc/environment file.
echo "LIGHTEN_DB_DSN='postgres://lighten:${DB_PASSWORD}@localhost/lighten'" >> /etc/environment
